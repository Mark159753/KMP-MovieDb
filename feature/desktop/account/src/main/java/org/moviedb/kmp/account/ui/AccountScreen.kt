...
@Composable
fun AccountScreen(
    state:AccountScreenState = koinInject()
) {
    val isInDarkModel by state.isDarkTheme.collectAsState(initial = isSystemInDarkTheme())
    val currentLng by state.currentLng.collectAsState(initial = AppLanguage.EN)
    ...
    Column(
        ...
        PreferenceSwitchRow(
            ...
            onCheckedChange = state::onIsDarkModelToggle,
